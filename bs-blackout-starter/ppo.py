...
class PPOAgent(BaseAgent):
    def __init__(
        self,
        model_path="ppo_bs_lstm_cuda_780m.zip",
        ...
        if action_mask_mode not in ("observable", "observable-hard"):
            raise ValueError(
                "action_mask_mode must be observable or observable-hard"
            )
    ...
    def move(self, game_state: GameState) -> MoveAction:
        ...
        # V17 can let the learned policy price visible head-to-head risk instead
        # of hard-coding conservative avoidance. Older exports keep the legacy
        # observable behavior by default.
        hard_legal, soft_legal = self._compute_legal(game_state)
        legal_model = (
            hard_legal
            if self.action_mask_mode == "observable-hard"
            else (soft_legal if soft_legal else hard_legal)
        )
...
if __name__ == "__main__":
    ...
    agent = PPOAgent(
        action_mask_mode=os.getenv("PPO_ACTION_MASK_MODE", "observable")
    )
    port = int(sys.argv[1])
