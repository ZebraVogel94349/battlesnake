...
def blackout_config() -> BattleSnakeConfig:
    """Creates a preset configuration for Battlesnake Blackout.

    Blackout uses the standard Battlesnake rules with imperfect information:
    four snakes play on a 15x15 board, each snake observes opponent segments and
    older food only within a Manhattan radius of 5 tiles around its head. Food
    spawned on the current turn remains globally visible for that turn.

    Returns:
        BattleSnakeConfig: The configured 4-player Blackout environment settings.
    """
    return restricted_standard_config()


def blackout_duel_config() -> BattleSnakeConfig:
    """Creates a 2-player Blackout preset for debugging and self-play tests.

    The tournament Blackout preset is :func:`blackout_config`; this duel variant
    keeps the same 15x15 board and view radius but only uses two snakes.

    Returns:
        BattleSnakeConfig: The configured 2-player Blackout environment settings.
    """
    return restricted_duel_config()
